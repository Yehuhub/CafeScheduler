...
- **Employees** never see past weeks — the home page shows only the current + upcoming published weeks.
...
| Published schedule — employee view | Employee home (`/`) shows the **current + upcoming** published weeks as accordion cards (current expanded, upcoming collapsed; fully-past weeks hidden — see §3 "Elapsed weeks"). Each card has a **My Shifts / Everyone** toggle. *My Shifts*: the employee's own shifts as a list. *Everyone*: a grid (days as columns, slots as rows, sticky slot column, horizontal scroll) with per-cell name chips color-coded by role (barista indigo, cook teal), own name highlighted. `GET /weeks/:weekId/assignments` also returns `assignees: [{id, name}]` so employees can render names without the boss-only user list. |
