...
A week is **past** once it has fully elapsed — today is on or after the following Sunday (`startDate + 7 days`). The current week (today still within its 7 days) is **not** past and stays fully editable. This is a pure date rule shared by server and client (`shared/weekDates.ts`: `isPastWeek` / `isCurrentWeek`).

- **Past weeks are frozen.** All mutations are rejected server-side with `409 WEEK_ENDED` regardless of status: assignment add/remove, requirements, weekly shift counts, status transitions, and **delete** (past weeks are immutable history). This narrows §3's "post-publish fixes are made by editing in place" — editing is allowed only until the week has fully elapsed.
- **Boss** keeps view + **PDF export** for past weeks. The dashboard hides them behind a "Show past weeks" toggle and renders them read-only (export only — no edit/delete affordances, and the schedule-health badge is suppressed).
- **Employees** view schedules read-only via the Schedule tab's week navigator, which defaults to the current week but can step ‹ › to any calendar week — **including past published weeks** (read-only; the backend still rejects any edit). Weeks with no published schedule show a message ("No schedule was made for this week" / "…yet" for future).
...
Any logged-in user, published-only (employees reach it via the Schedule tab's "Full view"
button; the boss via each week card). `export.html` renders the schedule grid as a
...
| PDF export (HTML print) | `GET /weeks/:weekId/export.html` — any logged-in user, published-only. Pure `server/src/services/scheduleExport.ts` (`buildScheduleView` + `renderScheduleHtml`, 11 Vitest tests) shapes assignments into a per-day/shift view and renders a standalone print-optimized page as horizontal **day columns** (one column per day; white shift cards with name + start time; role-colored name chips; landscape `@page`; print-color-adjust). Scrolls horizontally on phones, collapses to one page on print. Format→exporter registry; `export.pdf` stays a 501 stub reserved for a future binary. Boss dashboard shows a "Print / Export" link on published week cards; employees get a "Full view" button next to the My Shifts / Everyone toggle (both via `api.weeks.exportUrl`, open in a new tab). |
